...
// --- Configuration ---
// FIXED: Using 'lingaros.com' as per your working prototype reference
const LINGA_API_KEY =
  process.env.LINGA_API_KEY || "UiSg7JagVOd42IEwAnctfWS6qSTaKxxr";
...
app.get("/api/v1/lingapos/store/:storeId/getsale", async (req, res) => {
  try {
    ...
    res
      ...
      .json(error.data || { error: error.message });
  }
});
...
app.get(
  "/api/v1/lingapos/store/:storeId/saleSummaryReport",
  async (req, res) => {
    try {
      const { storeId } = req.params;
