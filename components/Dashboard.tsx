...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || "Failed to load data from server.");
      setData(null);
    } finally {
      setLoading(false);
      setSyncProgress("");
  ...
  const handleMainExport = () => {
    if (!data) return;
    const storeName =
      storeList.find((s) => s.id === selectedStore)?.name || "Store";
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 overflow-hidden transition-colors duration-300">
      ...
      <main className="flex-1 overflow-x-hidden overflow-y-auto">
        ...
        <div className="pb-8">
          {view === "OVERVIEW" && (
            <OverviewModule data={data} theme={theme} loading={loading} />
          )}
          {view === "REPORTS" && (
            <ReportsModule
              data={data}
