...
const DSRRecap = forwardRef<HTMLDivElement, DSRRecapProps>(
  (
    ...
    // Prepare Department Mix for table
    const departmentEntries = Object.entries(dsrStats.departmentMix || {}).sort(
      ...
      <div className="flex flex-col gap-6 animate-fadeIn pb-12" ref={ref}>
            ...
            <div className="p-4 grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full text-xs font-black text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-950 px-2 py-1.5 rounded border border-slate-100 dark:border-slate-800 outline-none focus:border-rose-500 no-print"
                />
                <span className="text-xs font-black print-only hidden">
                  {modName}
                </span>
              </div>
              <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded shadow-sm overflow-hidden">
            <h4 className="text-[10px] font-black uppercase text-white bg-[#001f3f] px-4 py-3 tracking-wider">
              Revenue Breakdown - Venue Level
            </h4>
            <table className="w-full text-xs">
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                <tr>
                  <td className="px-4 py-4 font-bold text-slate-900 dark:text-white uppercase text-[10px]">
                    Net Sales (Today)
                  </td>
                  <td className="px-4 py-4 text-right font-black text-slate-900 dark:text-white">
                    {formatAED(dsrStats.net)}
                  </td>
                  <td className="px-4 py-4 text-right text-slate-400 text-[10px] font-bold uppercase">
                    Revenue Base
                  </td>
                </tr>
                <tr>
                ...
                <tr className="bg-rose-50/50 dark:bg-rose-900/10">
                  <td className="px-4 py-6 font-bold text-slate-900 dark:text-white uppercase text-[10px]">
                    Gross Sales (Inc Tax)
                  </td>
                  <td className="px-4 py-6 text-right font-black text-rose-600 text-lg">
                    {formatAED(dsrStats.net + dsrStats.tax)}
                  </td>
                ...
                <tr className="bg-slate-50/50 dark:bg-slate-950/20">
                  <td className="px-4 py-4 text-slate-400 uppercase font-bold text-[10px]">
                    Operational Target (Net)
                  </td>
                  <td className="px-4 py-4 text-right">
                    <input
                      ...
                      placeholder="0.00"
                      className="w-24 bg-transparent border-b border-slate-300 dark:border-slate-700 text-right font-mono outline-none no-print"
                    />
                    <span className="print-only hidden">
                      {operationalTarget}
                    </span>
                  </td>
                  <td className="px-4 py-4 text-right text-slate-400 font-bold uppercase text-[10px]">
                    Budget Line
                  </td>
                </tr>
                <tr className="bg-slate-900 text-white font-bold">
                  <td className="px-4 py-4 uppercase text-[10px] text-slate-400">
                    Net Variance
                  </td>
            ...
            <table className="w-full text-xs">
              ...
              <tfoot className="bg-[#001f3f] text-white font-black">
                <tr>
                  <td
                    colSpan={2}
                    className="px-4 py-4 uppercase text-[10px] opacity-60"
                  >
                    Total Settlements
                  </td>
                  <td className="px-4 py-4 text-right text-sm text-rose-400">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded shadow-sm overflow-hidden">
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {departmentEntries.map(([dept, val], i) => {
                  const totalGrossMenu = departmentEntries.reduce(
                    (sum, entry) => sum + (entry[1] as number),
                    0
                  );
                  return (
                    <tr key={i}>
                      <td className="px-4 py-3 font-bold">{dept}</td>
                      <td className="px-4 py-3 text-right text-slate-400">
                        {totalGrossMenu > 0
                          ? (((val as number) / totalGrossMenu) * 100).toFixed(
                              1
                            )
                          : 0}
                        %
        ...
        <div className="flex justify-center pt-8 no-print">
          <button
            onClick={onExportPDF}
            disabled={exporting}
            className="bg-[#001f3f] text-white px-16 py-4 rounded-full font-black text-[11px] uppercase tracking-widest shadow-xl hover:scale-105 transition-all disabled:opacity-50"
          >
