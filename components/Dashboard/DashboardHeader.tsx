...
const DashboardHeader: React.FC<HeaderProps> = ({ title, isSimulated, theme, setTheme, desiredLiveMode, setDesiredLiveMode, onRefresh, onMenuClick }) => {
  return (
    <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 sticky top-0 z-30 px-8 py-4 flex justify-between items-center h-20 transition-colors duration-300">
      <div className="flex items-center gap-3">
        {onMenuClick && (
          <button 
            onClick={onMenuClick}
            className="md:hidden p-2 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:ring-2 ring-rose-500/20 transition-all text-slate-600 dark:text-slate-300"
          >
