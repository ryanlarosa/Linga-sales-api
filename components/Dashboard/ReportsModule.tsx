          ...
          {tab === 'RECAP' && dsrStats && (
            <div className="flex flex-col gap-8 animate-fadeIn pb-12" ref={recapRef}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                    ...
                    <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl transform transition-transform border border-slate-800">
                  ...
                  <div className="lg:col-span-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden shadow-sm h-fit">
                    ...
                    <table className="w-full text-[11px]">
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            <tr className="bg-slate-50 dark:bg-slate-950/40 font-black">
                                ...
                                <td className="px-4 py-4 text-right text-base">
                                  <div className="flex flex-col text-[10px] text-slate-500">
                                    <span>{dsrStats.checks} Checks</span>
                                    <span>{dsrStats.guestCount} Covers</span>
                                  </div>
                                </td>
                            </tr>
                            ...
                            <tr className={`${variance >= 0 ? 'text-emerald-600' : 'text-slate-900 dark:text-slate-100'} font-black bg-slate-50/20 dark:bg-slate-900/50`}>
                                <td className="px-4 py-4 uppercase tracking-widest text-[9px]">Target Variance</td>
                                <td className="px-4 py-4 text-right">{formatAED(variance)}</td>
                                <td className="px-4 py-4 text-right">{variancePct.toFixed(1)}%</td>
                            </tr>
                        </tbody>
                    </table>
                  </div>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {['Breakfast', 'Lunch', 'Dinner', 'Other'].map(key => (
                                    <tr key={key} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        ...
                                        <td className="px-4 py-3.5 text-right">{(dsrStats.segments as any)[key].checks}</td>
                                        <td className="px-4 py-3.5 text-right">{(dsrStats.segments as any)[key].covers}</td>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <table className="w-full text-[11px]">
                            ...
                            <tfoot className="bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800">
                                <tr className="font-black">
                                    <td className="px-4 py-4 text-[10px] uppercase tracking-wider">Consolidated Settlements</td>
                                    <td className="px-4 py-4"></td>
                                    <td className="px-4 py-4 text-right text-base text-slate-900 dark:text-white">
                                      {/* Using explicit arithmetic and type casting to ensure it stays a number */}
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden shadow-sm">
                        <h4 className="text-[10px] font-black uppercase text-white bg-[#001f3f] px-4 py-3">Discount Reduction Summary</h4>
                        <table className="w-full text-[11px]">
                            <thead className="bg-slate-50 dark:bg-slate-950 font-bold text-slate-500 border-b border-slate-100 dark:border-slate-800">
                                <tr>
                                    <th className="px-4 py-3 text-left">Campaign Name</th>
                                    <th className="px-4 py-3 text-right">Qty</th>
                                    <th className="px-4 py-3 text-right">Value (AED)</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                ...
                {/* 4. LOWER GRID: PRODUCT & CATEGORY MIX */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    <div className="lg:col-span-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden shadow-sm">
                        <h4 className="text-[10px] font-black uppercase text-white bg-[#001f3f] px-4 py-3">Top Product Performance</h4>
                        <table className="w-full text-[11px]">
                            <thead className="bg-slate-50 dark:bg-slate-950 font-bold text-slate-500 border-b border-slate-100 dark:border-slate-800 uppercase tracking-tighter">
                                <tr>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {menuPerformance.slice(0, 6).map((item, i) => (
                                    <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        <td className="px-4 py-3">
                                            <p className="font-bold text-slate-800 dark:text-slate-200">{item.name}</p>
                                            <p className="text-[9px] text-slate-400 uppercase font-black">{item.count} Sold</p>
                                        </td>
                                        <td className="px-4 py-3 text-right font-black font-mono text-slate-900 dark:text-white">{formatAED(item.value)}</td>
                                        <td className="px-4 py-3 text-right font-bold text-slate-400">{((item.value / dsrStats.net) * 100).toFixed(0)}%</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    ...
                    <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden shadow-sm">
                        <h4 className="text-[10px] font-black uppercase text-white bg-[#001f3f] px-4 py-3">Net Revenue Mix by Category</h4>
                        <table className="w-full text-[11px]">
                            <thead className="bg-slate-50 dark:bg-slate-950 font-bold text-slate-500 border-b border-slate-100 dark:border-slate-800">
                                <tr>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {Object.entries(dsrStats.categories).map(([k, v]) => (
                                    <tr key={k} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        <td className="px-4 py-4 font-bold text-slate-700 dark:text-slate-300">{k}</td>
                                        <td className="px-4 py-4 text-right font-bold text-slate-400">
                                            {dsrStats.net > 0 ? (v / dsrStats.net * 100).toFixed(0) : 0}%
                                        </td>
                                        <td className="px-4 py-4 text-right font-black font-mono text-slate-900 dark:text-white">
                                            {formatAED(v)}
                                        </td>
                                    </tr>
                                ))}
                ...
                <div className="flex justify-center pt-10 no-print">
                     ...
                     >
                        {exporting ? (
                          <><div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin mr-3"></div> Generating PDF Report...</>
                        ) : (
                          <><svg className="w-4 h-4 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg> Download Executive Recap (PDF)</>
                        )}
                    </button>
                </div>
          ...
          {tab === 'ANALYSIS' && (
            <div className="flex flex-col flex-1 gap-6 animate-fadeIn">
                <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                    ...
                    <button onClick={handleExportTab} className="bg-slate-900 hover:bg-black text-white px-5 py-2 rounded-xl text-xs font-bold transition-all shadow-xl">Export Report (Excel)</button>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm">
                        <ResponsiveContainer width="100%" height={320}>
                            ...
                            /* Re-typing Tooltip and Pie index mapping to resolve arithmetic and unknown assignment errors */
          ...
          {tab === 'MENU' && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm animate-fadeIn">
                <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950/40">
                    ...
                    <button onClick={handleExportTab} className="bg-slate-900 hover:bg-black text-white px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-lg">Export Sheet</button>
                </div>
                <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {menuPerformance.map((item, i) => (
                            <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-right font-medium text-slate-400">{item.count}</td>
