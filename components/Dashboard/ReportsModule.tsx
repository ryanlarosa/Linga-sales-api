    ...
    <div className="px-8 space-y-6 max-w-[1600px] mx-auto animate-fadeIn flex flex-col h-[calc(100vh-140px)] transition-colors duration-300">
      ...
      {data && (
        <div className="flex-1 overflow-auto custom-scrollbar pb-10">
          {tab === "RECAP" && dsrStats && (
            <div
              className="flex flex-col gap-8 animate-fadeIn pb-12"
              ref={recapRef}
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all">
                  ...
                  <h3 className="text-2xl font-black text-slate-600 dark:text-slate-300">
                    {formatAED(dsrStats.tax)}
                  </h3>
                </div>
                {/* Discount Card - Neutral Color (Slate) */}
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all">
                  <p className="text-[10px] font-black uppercase text-slate-400 mb-2 tracking-widest">
                    Discount
                  </p>
                  <h3 className="text-2xl font-black text-slate-700 dark:text-slate-200">
                ...
                <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm transition-all">
                  ...
                  <p className="text-[10px] text-slate-400 mt-1 uppercase font-bold tracking-tight">
                    Avg {formatAED(dsrStats.avgGuest)} / Guest
                  </p>
                </div>
                  ...
                  <table className="w-full text-[11px]">
                    <thead className="bg-slate-50 dark:bg-slate-950 font-bold text-slate-500 border-b border-slate-100 dark:border-slate-800">
                      <tr>
                        ...
                        <th className="text-right px-4 uppercase tracking-tighter">
                          Checks
                        </th>
                        <th className="text-right px-4 uppercase tracking-tighter">
                          Covers
                      ...
                      {["Breakfast", "Lunch", "Dinner", "Other"].map((key) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3.5 text-right font-mono text-slate-900 dark:text-slate-100">
                            {(dsrStats.segments as any)[
                              key
                            ].revenue.toLocaleString()}{" "}
                            AED
                          </td>
                          <td className="px-4 py-3.5 text-right font-bold text-slate-600 dark:text-slate-400">
                            {(dsrStats.segments as any)[key].checks}
                          </td>
                          <td className="px-4 py-3.5 text-right font-bold text-slate-600 dark:text-slate-400">
                            {(dsrStats.segments as any)[key].covers}
                          </td>
                          <td className="px-4 py-3.5 text-right font-bold text-slate-900 dark:text-slate-100">
                            {(dsrStats.segments as any)[key].covers > 0
                              ? formatAED(
                                  (dsrStats.segments as any)[key].revenue /
                                    (dsrStats.segments as any)[key].covers
                                )
                              : "0 AED"}
                          </td>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {Array.from(
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right font-black font-mono text-slate-700 dark:text-slate-300">
